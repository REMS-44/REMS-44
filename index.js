import { getPublicStudents } from "./public-data.js?v=6";
...
function render(){
  const visible=activeGroup?students.filter(s=>String(s.group||"")===activeGroup):students;
  ...
  studentGrid.innerHTML=visible.map(student=>{
    ...
    const initials=String(student.name||"?").split(/\s+/).slice(0,2).map(x=>x.charAt(0)).join("").toUpperCase();
    return `<a class="student-card" href="student.html?id=${encodeURIComponent(student.id)}">
      <div class="student-photo-wrapper">
        ${photo?`<img class="student-image" src="${esc(photo)}" alt="${esc(student.name)}" loading="lazy">`:`<div class="student-photo-placeholder">${esc(initials)}</div>`}
      </div>
  ...
  tabs?.querySelectorAll("button").forEach(btn=>btn.classList.toggle("active",btn.dataset.group===activeGroup));
}

if(tabs){
  tabs.innerHTML=[
    `<button type="button" class="student-group-tab active" data-group="">Усі <span>${students.length}</span></button>`,
    ...groups.map(g=>`<button type="button" class="student-group-tab" data-group="${esc(g)}">${esc(g)} <span>${students.filter(s=>String(s.group||"")===g).length}</span></button>`)
  ].join("");
  tabs.querySelectorAll("button").forEach(btn=>btn.onclick=()=>{activeGroup=btn.dataset.group||"";render();});
}
render();
