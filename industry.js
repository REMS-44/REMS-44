...
const card=(x,i)=>`<a class="meeting-card ${i===0?'meeting-card-featured':''}" href="meeting.html?id=${encodeURIComponent(x.id)}">
  <div class="meeting-cover">${x.cover?`<img src="${esc(x.cover)}" alt="">`:`<div class="meeting-cover-empty"></div>`}</div>
...
let items=[];
function render(){const a=[...items].sort((a,b)=>String(sort?.value==='old'?a.date:b.date||'').localeCompare(String(sort?.value==='old'?b.date:a.date||'')));grid.innerHTML=a.map(card).join('')}
try{
  items=await getMeetings();
  if(!items.length){grid.innerHTML='<div class="industry-empty"><span class="empty-number">00</span><div><b>Матеріали готуються</b><p>Незабаром тут з’являться перші зустрічі серії.</p></div></div>';} else render();
}catch(e){
  console.error(e);
